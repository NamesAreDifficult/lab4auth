...
import cv2
import os
trainPath = "C:\\Users\\kanga\\Documents\\fingerprints\\Train"
testPath = "C:\\Users\\kanga\\Documents\\fingerprints\\Test"
threshold = 0.63
loweRatio = 0.75
...
def fullTest():
    ...
    for findex, refFile in enumerate(trainFiles + testFiles):
        for sIndex, subFile in enumerate((trainFiles + testFiles)[findex:]):
            simPercent = compare(refFile, subFile, cv2.SIFT_create())
...
def sampleTest():
    ...
    for testNum in range(0,10):
        ...
        for i in range(0, math.floor(len(testRange)/10)):
            ...
            accscore = compare(refFile, refFile[0:-12] + 's' + refFile[-11:], cv2.SIFT_create())
            randFile = testRange[random.randint(0, len(testRange) - 1)]
            rejscore = compare(refFile, randFile[0:-12] + 's' + randFile[-11:], cv2.SIFT_create())
            for threshold in threshRange:
                threshold = threshold / 100
...
def frrTest():
    ...
    for file in trainFiles:
        simPercent = compare(trainPath + '\\' + file, trainPath + '\\' + file.replace('f', 's'), cv2.SIFT_create())
